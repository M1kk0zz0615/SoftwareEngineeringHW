...
2. **验收标准只有"满足需求条文"，没有"输出像不像小学题"**。需求列的全是硬约束
...
- **刘俊宁负责构思与审阅**：把需求里含糊的地方定死（"真分数"到底含不含带分数、
  "重复"到底怎么算）、定下数据结构与关键约束的实现思路、每次提交前对照需求逐条复查；
- **段旷卓负责代码实现**：把思路落成 C# 代码、搭内置自检、写基准程序量性能、
  整理 README 与提交历史。
...
第一次是**括号要不要存进语法树**。我构思时定的方向是：括号不占节点，渲染时按优先级算出来。
理由是需求文法里的 `(e)` 只是说"可以给任意子表达式加括号"，如果把括号存成节点，
反而要在渲染时再反过来判断"哪些括号是多余的"。段旷卓实现到渲染这一步时提出了一个我没想到的角度：
既然括号是渲染时算出来的，那么**渲染出来的题面必须能被解析回同一个值**，否则判题就会出错。
...
1. **实现时的"验收意识"**。他不只是把代码写出来，还会顺手把"这条思路怎么证明是对的"补上。
   上面第一次分歧就是例子：我定了"括号不渲染进树里"，他补的是"那渲染和解析必须互逆"，
   于是多出来一条能自动跑的断言。
2. **工程判断力：先量再改，并且愿意记录取舍**。性能那一节本来报一个
   "一万道题 68.9 毫秒"就能交差，他坚持要分段计时做归因，结果量出来查重结构是唯一的生死选择：
   `HashSet` 只要 1.55 ms，而同样的去重换成 `List` 线性查找要 589.9 ms，**慢 380 倍**。
   有了这个数，"为什么用哈希表"就从一句口头约定变成了有证据的设计决策。
   更难得的是，他提议把"被我们否掉的优化"（提前判重只能省 0.18 ms，却要把逻辑拆成两处）
   也写进文档——记录取舍才叫分析，只报数字那叫跑分。
3. **对交付物完整性的执念**。README 要能当运行说明用、目录结构要跟前两次作业对齐、
...
- **我审阅的覆盖面有盲区**。我复查的是"每条约束有没有被满足"，漏掉了"输入的写法有没有被覆盖"。
  ...
  教训是：用例要按**输入的结构**分类（分数在除号左侧、右侧、括号内、作为被减数……），
  而不是按"我想到了什么"来写。这是我这次最大的收获。
- **构思时没有把规模带进来**。设计方案的时候就应该先问一句"一万道题意味着什么"——
  10000 个元素去重该用什么结构？这个问题在设计阶段问出来，比实现完再测量便宜得多。
- **对"做完了没有"的判断太依赖需求条文**。第一版生成出来的题里出现了 `1 × 1 × 1 × 2`
